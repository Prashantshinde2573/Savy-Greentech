...
export function About() {
  ...
  return <main id="top" className="about-page">
    ...
    <section className="about-team section-cream" aria-labelledby="team-title"><div className="container"><div className="section-heading center"><h2 id="team-title">Our Team</h2><p className="about-section-intro">The people powering our progress.</p></div><div className="about-team-grid">{leadershipTeam.map(({ name, role, image, linkedin, placeholder }) => <article key={name}><div className="team-card-media"><img src={image} alt={placeholder ? '' : `${name}, ${role}`} loading="lazy" />{linkedin && <a className="team-linkedin" href={linkedin} target="_blank" rel="noreferrer" aria-label={`${name} on LinkedIn`}><PiLinkedinLogo aria-hidden="true" /></a>}</div><div className="team-card-copy"><h3>{name}</h3><p className="team-role">{role}</p></div></article>)}</div></div></section>
